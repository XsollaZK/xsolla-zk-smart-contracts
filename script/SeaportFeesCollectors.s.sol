...
import { EthereumFeeCollector } from "src/collector/EthereumFeeCollector.sol";
...
contract SeaportFeesCollectors is Autowirable {
    ...
    /// @param amount Amount to withdraw
    function withdrawERC20FromBaseFeeCollector(address withdrawalWallet, address tokenContract, uint256 amount)
        external
    {
        if (address(baseFeeCollector) == address(0)) {
            revert BaseFeeCollectorNotDeployed();
