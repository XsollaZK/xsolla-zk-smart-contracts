...
contract BaseFeeCollector is Ownable, BaseFeeCollectorInterface, BaseFeeCollectorEventsAndErrors {
    ...
    function withdrawERC20Tokens(address withdrawalWallet, address tokenContract, uint256 amount) external override isOperator {
        ...
        (bool result, bytes memory data) = tokenContract.call(
            abi.encodeWithSelector(IERC20.transfer.selector, withdrawalWallet, amount)
        );

        // Revert if we have a false result.
        if (!result) {
            revert TokenTransferGenericFailure(tokenContract, withdrawalWallet, 0, amount);
        }

        // Revert if we have a bad return value.
        if (data.length != 0 && data.length >= 32) {
            if (!abi.decode(data, (bool))) {
                revert BadReturnValueFromERC20OnTransfer(tokenContract, withdrawalWallet, amount);
            }
        }
    }
    ...
    function withdraw(address withdrawalWallet, uint256 amount) external override isOperator {
        // Ensure the withdrawal wallet is in the withdrawal wallet mapping.
        if (_withdrawalWallets[withdrawalWallet] != true) {
            revert InvalidWithdrawalWallet(withdrawalWallet);
        }
        ...
        Address.sendValue(payable(withdrawalWallet), amount);
    }
    ...
    function addWithdrawAddress(address newWithdrawalWallet) external override onlyOwner {
    ...
    ///                             remove.
    function removeWithdrawAddress(address withdrawalWallet) external override onlyOwner {
    ...
    function assignOperator(address operatorToAssign) external override onlyOwner {
    ...
    function isWithdrawalWallet(address withdrawalWallet) external view override returns (bool) {
    ...
    function _setWithdrawalWallet(address withdrawalAddress, bool valueToSet) internal {
