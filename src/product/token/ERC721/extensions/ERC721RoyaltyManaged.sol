...
contract ERC721RoyaltyManaged is ERC721Enumerable, ERC721Royalty, ERC721URIStorage, AccessControl, Pausable, IEIP721Mintable {
    ...
    function batchSetTokenURIs(uint256[] calldata tokenIds, string[] calldata _tokenURIs) external onlyRole(URI_SETTER_ROLE) {
    ...
    function batchMint(uint256[] calldata amounts, address[] calldata recipients) external onlyRole(MINTER_ROLE) whenNotPaused {
    ...
    function maxSupply() external view returns (uint256) { return config.maxSupply; }
    
    /// @notice Get the current price per token in wei
    /// @return The token price for public minting
    function tokenPrice() external view returns (uint256) { return config.tokenPrice; }
    
    /// @notice Get the maximum tokens allowed per transaction
    /// @return The per-transaction limit (0 means unlimited)
    function maxPerTransaction() external view returns (uint256) { return config.maxPerTransaction; }
    
    /// @notice Check if public token sales are currently active
    /// @return True if public minting is enabled
    function saleIsActive() external view returns (bool) { return config.saleIsActive; }
    
    /// @notice Get the current royalty percentage in basis points
    /// @return The royalty factor (10000 = 100%)
    function royaltyFactorBps() external view returns (uint96) { return config.royaltyFactorBps; }
    ...
    /// @dev Override _burn to clear royalty info and token URI
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC721Enumerable, ERC721Royalty, ERC721URIStorage, AccessControl) returns (bool) {
        return
