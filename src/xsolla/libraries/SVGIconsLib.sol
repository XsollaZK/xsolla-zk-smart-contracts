...
library SVGIconsLib {
    ...
    function getIcon(
        ...
        bytes memory svg = _makeSvg(name, tokenId, fields);

        return _makeJson(name, externalUrl, shortDescription, svg, attrs);
    }
    ...
    function _makeSvg(string memory name, string memory tokenId, Field[8] memory fields)
        internal
        pure
        returns (bytes memory)
    {
        ...
        return abi.encodePacked(
            head,
            abi.encodePacked(
                '<text x="8" y="30" class="tag">', name, "</text>", '<text x="8" y="335">Token ID: ', tokenId, "</text>"
            ),
            items,
            "</svg>"
        );
    ...
    function _textTag(string memory _prefixTxt, string memory _txt, string memory _yPos)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encodePacked('<text x="8" y="', _yPos, '">', _prefixTxt, _txt, "</text>");
    }
    ...
    function _makeAttr(string memory _k, string memory _v, string memory _d) internal pure returns (bytes memory) {
        if (keccak256(abi.encodePacked(_d)) == keccak256(abi.encodePacked("none"))) {
            return abi.encodePacked('{"trait_type": "', _k, '", "value": "', _v, '"}');
        } else if (keccak256(abi.encodePacked(_d)) == keccak256(abi.encodePacked("ranking"))) {
            return abi.encodePacked('{"trait_type": "', _k, '", "value": ', _v, "}");
        } else {
            return abi.encodePacked('{"trait_type": "', _k, '", "value": "', _v, '", "display_type": "', _d, '"}');
        }
    }
}
