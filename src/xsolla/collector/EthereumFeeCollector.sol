...
contract EthereumFeeCollector is BaseFeeCollector, EthereumFeeCollectorInterface {
    ...
    function unwrapAndWithdraw(address withdrawalWallet, address wrappedTokenContract, uint256 amount)
        external
        override
        isOperator
    {
        ...
        (bool result, bytes memory data) =
            wrappedTokenContract.call(abi.encodeWithSelector(IWETH.withdraw.selector, amount));

        // Revert if we have a false result.
        if (!result) {
            revert TokenTransferGenericFailure(wrappedTokenContract, withdrawalWallet, 0, amount);
        }

        // Revert if we have a bad return value.
        if (data.length != 0 && data.length >= 32) {
            if (!abi.decode(data, (bool))) {
                revert BadReturnValueFromERC20OnTransfer(wrappedTokenContract, withdrawalWallet, amount);
            }
        }

        // Transfer the now unwrapped tokens to the withdrawal address.
        payable(withdrawalWallet).sendValue(amount);
    }
