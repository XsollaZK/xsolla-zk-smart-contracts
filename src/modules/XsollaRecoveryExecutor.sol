...
/// @notice Extends GuardianExecutor by introducing an implicit global Xsolla guardian that can recover any account without being explicitly registered per account.
/// Guardian management (add / accept / remove) is intentionally disabled; only the implicit guardian is recognized.
...
contract XsollaRecoveryExecutor is GuardianExecutor, AccessControl {
    ...
    /// @notice Address of the implicit global Xsolla guardian allowed to initiate recovery for any account.
    address public immutable xsollaGuardian;
    ...
    event XsollaGuardianRecovery(
        address indexed account,
        address indexed guardian
    );

    /// @notice Error indicating the caller is not the implicit Xsolla guardian where required.
    error NotXsollaGuardian();
    /// @notice Error thrown when a zero address is provided for the Xsolla guardian during construction.
    error XsollaGuardianZero();
    ...
    /// @param _xsollaGuardian The implicit global guardian address (must be non-zero).
    ...
    constructor(
        ...
        address _xsollaGuardian,
        address _finalizer,
        address _admin,
        address _submitter
    ) GuardianExecutor(_webAuthValidator, _eoaValidator) {
        if (_xsollaGuardian == address(0)) revert XsollaGuardianZero();
        xsollaGuardian = _xsollaGuardian;
    ...
    function initializeRecovery(
        ...
        bytes calldata data
    ) external virtual override onlyRole(SUBMITTER_ROLE) {
        ...
        uint256 pendingRecoveryTimestamp = pendingRecovery[accountToRecover]
            .timestamp;
        if (
            !(pendingRecoveryTimestamp == 0 ||
                pendingRecoveryTimestamp + REQUEST_VALIDITY_TIME <
                block.timestamp)
        ) {
            revert RecoveryInProgress(accountToRecover);
        }
        RecoveryRequest memory recovery = RecoveryRequest(
            recoveryType,
            data,
    ...
    function finalizeRecovery(
        address account
    ) external virtual override onlyRole(FINALIZER_ROLE) returns (bytes memory returnData) {
        ...
        if (
            !(recovery.timestamp +
                SafeCast.toUint256(
                    SafeCast.toInt256(REQUEST_DELAY_TIME) +
                        requestDelayTimeOffset
                ) <
                block.timestamp &&
                recovery.timestamp +
                    SafeCast.toUint256(
                        SafeCast.toInt256(REQUEST_VALIDITY_TIME) +
                            requestValidityTimeOffset
                    ) >
                block.timestamp)
        ) {
        ...
        address validator = recovery.recoveryType == RecoveryType.EOA
            ? eoaValidator
            : webAuthValidator;
        // slither-disable-next-line incorrect-equality
        ...
        bytes memory execution = ExecutionLib.encodeSingle(
            validator,
            0,
            abi.encodePacked(selector, recovery.data)
        );
    ...
    function setRequestDelayTimeOffset(
        int256 offset
    ...
    /// @dev Only callable by admin.
    /// @param offset The signed offset in seconds (positive or negative).
    function setRequestValidityTimeOffset(
        int256 offset
    ) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
