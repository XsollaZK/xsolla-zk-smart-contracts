...
contract ERC721ModularFuzzTest is Test {
    ...
    function setUp() public {
        token = new ERC721Modular("Test NFT", "TEST", 10000);
        token.grantRole(token.MINTER_ROLE(), minter);
        token.toggleMinting();
    }

    function testFuzz_MintTokens(
        address to,
        uint256 amount
    ) public {
    ...
    function testFuzz_ApproveAndTransfer(
        address owner,
        address spender,
        address to
    ) public {
    ...
    function testFuzz_TokenURIGeneration(uint256 tokenId) public {
        ...
        assertNotEq(
            keccak256(bytes(uriWithoutSvg)),
            keccak256(bytes(uriWithSvg))
        );
    }
